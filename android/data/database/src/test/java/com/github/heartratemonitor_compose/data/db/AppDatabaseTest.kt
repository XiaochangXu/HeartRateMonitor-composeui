...
class AppDatabaseTest {
    ...
    @Test
    fun `getSampledHeartRatesForSession with step 1 returns all values`() = runTest {
        val sessionId = heartRateDao.insertSession(HeartRateSession(deviceName = "A", startTime = 1000L))
        heartRateDao.insertRecord(HeartRateRecord(sessionId = sessionId, timestamp = 1000L, heartRate = 60))
        heartRateDao.insertRecord(HeartRateRecord(sessionId = sessionId, timestamp = 2000L, heartRate = 70))
        heartRateDao.insertRecord(HeartRateRecord(sessionId = sessionId, timestamp = 3000L, heartRate = 80))

        val samples = heartRateDao.getSampledHeartRatesForSession(sessionId, step = 1)
        assertThat(samples).hasSize(3)
        assertThat(samples[0]).isEqualTo(60)
        assertThat(samples[1]).isEqualTo(70)
        assertThat(samples[2]).isEqualTo(80)
    }

    @Test
    fun `getSampledHeartRatesForSession with step 2 returns every other value`() = runTest {
        val sessionId = heartRateDao.insertSession(HeartRateSession(deviceName = "A", startTime = 1000L))
        // 插入 6 条记录，step=2 → 采样第 1,3,5 条（rn=1,3,5）
        heartRateDao.insertRecord(HeartRateRecord(sessionId = sessionId, timestamp = 1000L, heartRate = 60))
        heartRateDao.insertRecord(HeartRateRecord(sessionId = sessionId, timestamp = 2000L, heartRate = 65))
        heartRateDao.insertRecord(HeartRateRecord(sessionId = sessionId, timestamp = 3000L, heartRate = 70))
        heartRateDao.insertRecord(HeartRateRecord(sessionId = sessionId, timestamp = 4000L, heartRate = 75))
        heartRateDao.insertRecord(HeartRateRecord(sessionId = sessionId, timestamp = 5000L, heartRate = 80))
        heartRateDao.insertRecord(HeartRateRecord(sessionId = sessionId, timestamp = 6000L, heartRate = 85))

        val samples = heartRateDao.getSampledHeartRatesForSession(sessionId, step = 2)
        assertThat(samples).hasSize(3)
        assertThat(samples[0]).isEqualTo(60)  // rn=1
        assertThat(samples[1]).isEqualTo(70)  // rn=3
        assertThat(samples[2]).isEqualTo(80)  // rn=5
    }

    @Test
    fun `getSampledHeartRatesForSession returns empty for no records`() = runTest {
        val sessionId = heartRateDao.insertSession(HeartRateSession(deviceName = "A", startTime = 1000L))
        val samples = heartRateDao.getSampledHeartRatesForSession(sessionId, step = 1)
