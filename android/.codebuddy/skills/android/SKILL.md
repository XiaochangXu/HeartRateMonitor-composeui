...
description: Android 心率监测项目开发规范。当新增或修改 Kotlin/Compose 代码、调整分层与模块边界、增删 DataStore 设置项、改动 BLE 心率数据流或落盘链路、处理 WindowInsets 避让与 i18n 数字渲染、或改完需执行 gradlew 验证基线时，必须先加载本 Skill。内含 13 条禁止破坏的架构契约、注释与代码风格红线、单测基线命令与已知 flaky 清单。
---
