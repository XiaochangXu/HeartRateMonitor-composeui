...
class AppContainer(private val application: Application) {
    ...
    /** 应用级 Room 数据库单例，统一管理，消除散布的 AppDatabase.getDatabase() 直接调用。 */
    val appDatabase: AppDatabase by lazy { AppDatabase.getDatabase(application) }
    ...
    val favoriteDeviceRepository: FavoriteDeviceRepository by lazy { FavoriteDeviceRepository(application) }

    val sessionRepository: SessionRepository by lazy { SessionRepository(application) }
