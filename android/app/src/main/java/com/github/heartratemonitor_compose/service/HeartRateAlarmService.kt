...
class HeartRateAlarmService : Service() {
    ...
    /** 新鲜度订阅协程，随 heartRateJob 一同管理生命周期 */
    ...
    override fun onCreate() {
        ...
        postureDetector.setCalibration(
            PostureCalibration.fromJson(sharedPreferences.getString(PrefsKeys.POSTURE_CALIBRATION_DATA, null))
        )

        val high = sharedPreferences.getInt(PrefsKeys.HEART_RATE_ALARM_HIGH_THRESHOLD, 100)
        val low = sharedPreferences.getInt(PrefsKeys.HEART_RATE_ALARM_LOW_THRESHOLD, 50)
        val dur = sharedPreferences.getInt(PrefsKeys.HEART_RATE_ALARM_DURATION_SECONDS, 10)
        ...
        excludePostureDetection = sharedPreferences.getBoolean(PrefsKeys.HEART_RATE_ALARM_EXCLUDE_POSTURE_DETECTION, false)
    ...
    private val settingsListener = SharedPreferences.OnSharedPreferenceChangeListener { _, key ->
        when (key) {
            PrefsKeys.HEART_RATE_ALARM_HIGH_THRESHOLD,
            PrefsKeys.HEART_RATE_ALARM_LOW_THRESHOLD,
            PrefsKeys.HEART_RATE_ALARM_DURATION_SECONDS,
            PrefsKeys.HEART_RATE_ALARM_REPEAT_ENABLED,
            PrefsKeys.HEART_RATE_ALARM_REPEAT_INTERVAL_MINUTES -> reloadAlarmConfig()
            PrefsKeys.HEART_RATE_ALARM_EXCLUDE_POSTURE_DETECTION -> {
                excludePostureDetection = sharedPreferences.getBoolean(PrefsKeys.HEART_RATE_ALARM_EXCLUDE_POSTURE_DETECTION, false)
                applyPostureDetectionState()
            }
            PrefsKeys.POSTURE_CALIBRATION_DATA -> {
                postureDetector.setCalibration(
                    PostureCalibration.fromJson(
                        sharedPreferences.getString(PrefsKeys.POSTURE_CALIBRATION_DATA, null)
                    )
                )
            }
        }
    ...
    private fun computeEffectiveCooldown(): Long {
        val isRepeatEnabled = sharedPreferences.getBoolean(PrefsKeys.HEART_RATE_ALARM_REPEAT_ENABLED, false)
        val intervalMinutes = sharedPreferences.getInt(PrefsKeys.HEART_RATE_ALARM_REPEAT_INTERVAL_MINUTES, 5)
