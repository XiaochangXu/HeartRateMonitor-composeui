...
class BleService : Service(), FairMemoryReceiver.MemoryListener {
        ...
        serviceScope.launch {
            heartRateFreshnessTracker.freshness.collect { state ->
                ...
                if (state == HeartRateFreshness.STALE &&
                    ...
                    Log.w("BleService", "心率数据长时间未更新，判定测量失败并清零")
                    _heartRate.value = 0
    ...
    fun startScan(durationMillis: Long = 15_000L) {
        ...
        val useFilter = sharedPreferences.getBoolean(PrefsKeys.SCAN_FILTER_ENABLED, false)
                ...
                peripheral = Peripheral(identifier) {
                    ...
                    onServicesDiscovered {
                        ...
                        try {
                            ...
                            Log.w("BleService", "MTU negotiation failed, using default", e)
                        }
                    }
                }
        ...
        when (state) {
            ...
            is State.Connected -> {
                ...
                try {
                    ...
                    Log.w("BleService", "requestConnectionPriority failed", e)
                }
    ...
    private suspend fun cleanupConnection(peripheral: Peripheral?, epoch: Long) {
        ...
        heartRateFreshnessTracker.reset()
        ...
        while (currentCoroutineContext().isActive) {
            try {
                ...
                bleManager.observeHeartRate(peripheral).collect { measurement ->
                    heartRateFreshnessTracker.onPacket()
