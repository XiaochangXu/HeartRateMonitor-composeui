...
- 当前单测基线：**全部通过（213 用例，0 失败）**；其中 29 个为纯 UDF 迁移（2026-08）新增的设置页 ViewModel 往返一致性/配对状态机用例；教科书式 MVI 迁移（2026-08）完成后新增 MviViewModel 基类测试 ×2 与阈值 clamp 纯归约测试 ×2，验收存档见 `baseline/mvi-baseline.md`。历史基线问题（AppDatabaseTest / HeartRateRecorderTest 的 Room 3→4 迁移缺失导致的 28 个预存失败）已于 2026-08 修复——测试改用 Room 默认驱动（RoomOpenHelper 自行建表与版本管理），不再手工提供 SupportSQLiteOpenHelper + 空 onCreate Callback；出现新失败必须修复。
...
## 10. 纯 UDF / 教科书式 MVI 状态（禁止回退）

以下约束固化纯 UDF 迁移与教科书式 MVI 迁移（依据《Android教科书式MVI渐进式迁移方案.md》，2026-08 完成）的成果，违反即视为破坏架构：
