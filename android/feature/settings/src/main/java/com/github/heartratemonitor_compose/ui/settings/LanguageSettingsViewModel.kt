...
class LanguageSettingsViewModel @Inject constructor(
    ...
    LanguageSettingsUiState(
        selectedLanguage = normalizeLanguageTag(settings.getNullable(SettingsKeys.APP_LANGUAGE))
    )
) {
    ...
    init {
        viewModelScope.launch {
            settings.observeNullable(SettingsKeys.APP_LANGUAGE).collect { lang ->
                setState { it.copy(selectedLanguage = normalizeLanguageTag(lang)) }
            }
        }
    }
    ...
    companion object {
        /**
         * 规范化语言 Tag：将旧版本存储的印尼语 BCP 47 标准代码 "id"
         * 转换为 Java/Android 实际使用的旧代码 "in"，
         * 使 UI 选中状态与 LANGUAGE_OPTIONS 中的 tag 一致。
         * （LocaleHelper 中也做了同样的规范化用于运行时 locale 应用）
         */
        private fun normalizeLanguageTag(tag: String?): String? {
            return if (tag == "id") "in" else tag
        }
    }
}
