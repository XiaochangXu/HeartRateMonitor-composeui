...
class MainViewModel @Inject constructor(
    ...
    internal val chartDataManager = ChartDataManager(viewModelScope)
    ...
    init {
        ...
        viewModelScope.launch {
            settings.observe(SettingsKeys.HISTORY_RECORDING_ENABLED).collect { enabled ->
        ...
        viewModelScope.launch {
            settings.observe(SettingsKeys.SPEED_DISPLAY_ENABLED).collect {
                setState { s -> s.copy(isSpeedEnabled = it) }
            }
        }
        viewModelScope.launch {
            settings.observe(SettingsKeys.SCAN_FILTER_ENABLED).collect {
                setState { s -> s.copy(scanFilterEnabled = it) }
            }
        }
        viewModelScope.launch {
            settings.observe(SettingsKeys.HEART_RATE_RING_MAX).collect {
                setState { s -> s.copy(ringMaxHr = it) }
            }
        }
        viewModelScope.launch {
            settings.observe(SettingsKeys.FLOATING_WINDOW_ENABLED).collect {
                setState { s -> s.copy(floatingWindowEnabled = it) }
            }
        }
        viewModelScope.launch {
            settings.observe(SettingsKeys.HIDE_FROM_RECENTS_ENABLED).collect {
                setState { s -> s.copy(hideFromRecentsEnabled = it) }
            }
        }
        viewModelScope.launch {
            settings.observe(SettingsKeys.SEARCH_TIP_SHOWN).collect {
                setState { s -> s.copy(searchTipShown = it) }
            }
        }
        viewModelScope.launch {
            settings.observe(SettingsKeys.HEARTBEAT_ANIMATION_ENABLED).collect {
                setState { s -> s.copy(heartbeatAnimationEnabled = it) }
            }
        }
        viewModelScope.launch {
            // 全屏心率文字色（历史分歧点：全屏页默认 RED，保留显式默认值重载）
            settings.observe(SettingsKeys.FLOATING_TEXT_COLOR, android.graphics.Color.RED).collect {
                setState { s -> s.copy(fullscreenHeartTextColor = it) }
            }
        }
        viewModelScope.launch {
            settings.observeNullable(SettingsKeys.FAVORITE_DEVICE_ID).collect {
                setState { s -> s.copy(favoriteDeviceId = it) }
            }
        }
        viewModelScope.launch {
            // 构造期解析旧开关迁移（等价旧全屏页进入时调用 resolveSoundMode，仅键缺失时写入，幂等）
            val resolved = resolveSoundModeFallback
            settings.observeNullable(SettingsKeys.FULLSCREEN_SOUND_MODE).collect { mode ->
                setState { s -> s.copy(fullscreenSoundMode = mode ?: resolved) }
            }
        }
    ...
    fun cleanupOpenSessions() {
        viewModelScope.launch {
            sessionRepository.closeOpenSessions()
        }
    }
