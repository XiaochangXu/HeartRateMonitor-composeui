...
fun HeartRateAlarmScreen(
    ...
    DisposableEffect(excludePostureDetection) {
        if (excludePostureDetection) {
            viewModel.dispatch(HeartRateAlarmIntent.StopPostureDetection)
            onDispose { }
        } else {
            viewModel.dispatch(HeartRateAlarmIntent.StartPostureDetection)
            onDispose { viewModel.dispatch(HeartRateAlarmIntent.StopPostureDetection) }
        }
    }
