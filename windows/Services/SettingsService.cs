...
public static class SettingsService
{
    ...
    private static FloatWindowSettings Load()
    {
        try
        {
            ...
            {
                var settings = JsonSerializer.Deserialize<FloatWindowSettings>(File.ReadAllText(FilePath), _jsonOptions);
                if (settings is not null)
                {
                    // 迁移：老版本（显式/默认）的 Mica 统一切到亚克力——Mica 仅 Win11 支持。
                    // 直接落盘让磁盘与内存一致，之后再次加载就不会重复迁移（幂等）。
                    if (settings.MigrateMicaToAcrylicBackdrop())
                    {
                        try
                        {
                            var dir = Path.GetDirectoryName(FilePath)!;
