...
public sealed class HeartRateService : IDisposable
{
    ...
    {
        // 应用设置开启"搜索过滤"时，只处理声明了心率服务 (0x180D) 的广播包，
        // 避免耳机/手环等无关 BLE 设备刷屏设备列表
        if (SettingsService.Current.FilterHeartRateOnly
            && !args.Advertisement.ServiceUuids.Contains(GattServiceUuids.HeartRate))
            return;
